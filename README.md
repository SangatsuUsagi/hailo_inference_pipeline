...
- **Post-Processing Support**: Built-in support for classification and mediapipe palm detection models
...
| `--postprocess`  | `-p`  | Type of post processing (classification or palm_detection) | `classification`              |
| `--label`        | `-l`  | Path to label definition JSON file                         | `./postprocess/imagenet.json` |
...
```bash
python inference.py image.jpg -p classification -l my_labels.json -n ./hefs/your_custom_model.hef
```
...
```bash
python inference.py hand.jpg -n ./hef/palm_detection_full.hef -p palm_detection
```

#### Run palm detection on a video with asynchronous inference and using callback function:

```bash
python inference.py vodeo.mp4 -n ./hef/palm_detection_full.hef -p palm_detection --asynchronous --callback
```
