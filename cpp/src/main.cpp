...
// ---------------------------------------------------------------------------
struct VStreamInfo { std::string name; int H, W; bool is_nms; };

static std::vector<VStreamInfo> print_vstream_infos(
    const std::vector<hailo_vstream_info_t>& infos, bool is_input)
{
    ...
    for (size_t i = 0; i < infos.size(); ++i) {
        ...
        std::cout << kind << " #" << i << " " << vi.name
                  << (nms ? " [NMS]" : "")
                  << " (" << vi.shape.height << "x" << vi.shape.width
                  << "x" << vi.shape.features << ")\n";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        infer = std::make_unique<InferPipeline>(
            args.net_path, args.batch_size,
            is_async, is_callback, is_nms,
        ...
        if (args.postprocess_type == "nms_on_host") {
            ...
            std::string cfg = args.config_path.empty() ? "./configs/palm_detection_full.json" : args.config_path;
            postprocessor = std::make_unique<ImagePostprocessorPalmDetection>(pp_params, cfg);
        } else { // classification (default)
            std::string cfg = args.config_path.empty() ? "./configs/class_names_imagenet.json" : args.config_path;
        ...
        if (args.profile) std::cout << "Profiling enabled.\n";
        std::cout << "Press 'q' to quit\n\n";

        while (loop) {
            if (args.profile) profiler.start_frame();
            ++frame_count;
            ...
            if (is_image) {
                ret = cap->read(frame);
                if (args.profile) profiler.checkpoint("1_frame_read");
            } else {
            ...
            if (args.profile) profiler.checkpoint("2_preprocessing");

            try {
                // --- Inference ---
                auto outputs = infer->inference({input_frame});
                if (args.profile) profiler.checkpoint("3_inference_submit");

                if (is_image && is_async) {
                    last_outputs = infer->wait_and_get_output();
                    if (args.profile) profiler.checkpoint("4_inference_wait");
                } else if (!is_async) {
                ...
                if (args.profile) profiler.checkpoint("5_postprocessing");

                if (!is_image && is_async) {
                    last_outputs = infer->wait_and_get_output();
                    if (args.profile) profiler.checkpoint("6_inference_wait");
                }

                // --- Display ---
                if (is_image) {
                    cv::imshow("Output", out_frame);
                    if (args.profile) profiler.checkpoint("7_display");
                    loop = false;
                    cv::waitKey(0);
                } else {
                    if (display_thread) {
                        ...
                        if (display_thread->is_quit_requested()) loop = false;
                    }
                }
            ...
            if (args.profile) profiler.end_frame();
        }
    ...
    if (!is_image) {
        ...
        if (overall_elapsed > 0)
            std::cout << "Overall throughput: "
                      << frame_count / overall_elapsed << " FPS\n";
        std::cout << std::string(80, '=') << "\n";
    }
