...
void PerformanceProfiler::export_perfetto_trace(const std::string &output_path) const {
  ...
  for (const auto &name : frame_order) {
    ...
    if (name.rfind("display_", 0) == 0)
      display_cps.push_back(name);
    else if (name.rfind("capture_", 0) == 0)
      capture_cps.push_back(name);
    else
      main_cps.push_back(name);
  }
  ...
  auto add_events = [&](const std::vector<std::string> &cps, int tid, const std::string &cat,
    ...
    for (size_t f = 0; f < num_frames; ++f) {
      ...
      if (tid == main_thread_id) {
        trace_events.push_back({{"name", "Frame " + std::to_string(f + 1)},
                                {"cat", "frame"},
                                {"ph", "i"},
                                {"ts", start_us},
                                {"pid", process_id},
      ...
      for (const auto &cp : cps) {
        ...
        trace_events.push_back({{"name", display_name},
                                {"cat", cat},
                                {"ph", "X"},
                                {"ts", start_us},
                                {"dur", dur_us},
  ...
  // Metadata
  trace_events.push_back({{"name", "process_name"},
                          {"ph", "M"},
                          {"pid", process_id},
                          {"args", {{"name", "Hailo Inference Pipeline"}}}});
  trace_events.push_back({{"name", "thread_name"},
                          {"ph", "M"},
                          {"pid", process_id},
                          {"tid", main_thread_id},
...
bool DisplayThread::is_quit_requested() const { return quit_requested_; }

void DisplayThread::collect_timing_data() {
  if (!profiler_)
    return;
  TimingData td;
  if (!timing_queue_.try_pop(td))
    return;
...
void FrameReaderThread::read_loop(std::stop_token stoken) {
  while (!stoken.stop_requested()) {
    ...
    if (!ok || frame.empty()) {
      read_error_ = !ok;
      frame_queue_.push(std::nullopt,
                        milliseconds(1000)); // end-of-video sentinel
      break;
    }
    ...
    if (!pushed)
      continue; // drop frame if queue stayed full
...
cv::Mat FrameReaderThread::get_frame(int timeout_ms) {
  ...
  if (!got || !item.has_value())
    return {}; // empty Mat signals end/timeout
  return *item;
}

bool FrameReaderThread::has_error() const { return read_error_; }
