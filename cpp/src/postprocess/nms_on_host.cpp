...
ImagePostprocessorNmsOnHost::ImagePostprocessorNmsOnHost(
  ...
  pads_ = {static_cast<float>(params.second.first), static_cast<float>(params.second.second)};
  scales_ = {in_h * params.first.first, in_w * params.first.second};
...
std::array<float, 4>
ImagePostprocessorNmsOnHost::denormalize_box(const std::array<float, 4> &box) const {
  ...
  return {box[0] * scales_.first - pads_.first, box[1] * scales_.second - pads_.second,
          box[2] * scales_.first - pads_.first, box[3] * scales_.second - pads_.second};
}
...
cv::Mat ImagePostprocessorNmsOnHost::draw_detections(cv::Mat frame, const OutputMap &outputs) {
  if (outputs.empty())
    return frame;

  const auto &[first_name, first_output] = *outputs.begin();
  if (!first_output.is_nms)
    return frame;
  ...
  for (size_t class_id = 0; class_id < nms.size(); ++class_id) {
    for (const auto &det : nms[class_id]) {
      ...
      int baseline = 0;
      cv::Size text_sz =
          cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, font_scale, text_thickness, &baseline);

      cv::Point bg_tl = top_left;
      cv::Point bg_br(top_left.x + text_sz.width, top_left.y + text_sz.height + baseline);
      cv::rectangle(frame, bg_tl, bg_br, line_color, cv::FILLED);

      cv::putText(frame, label, cv::Point(top_left.x, top_left.y + text_sz.height),
...
cv::Mat ImagePostprocessorNmsOnHost::postprocess(const cv::Mat &frame, const OutputMap &outputs) {
  if (outputs.empty())
    return frame;
