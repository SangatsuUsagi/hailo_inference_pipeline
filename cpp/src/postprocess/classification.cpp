...
cv::Mat ImagePostprocessorClassification::add_text_to_image(
    cv::Mat image, const std::vector<std::string>& strings)
{
    ...
    for (size_t i = 0; i < strings.size(); ++i) {
        ...
        cv::putText(image, strings[i], cv::Point(x_start, y),
                    font, scale, color, thickness);
    }
    return image;
}

cv::Mat ImagePostprocessorClassification::postprocess(
    const cv::Mat& frame, const OutputMap& outputs)
{
    ...
    for (const auto& [key, tensor] : outputs) {
        if (tensor.is_nms || tensor.regular.data.empty()) continue;
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            display_strings.push_back("#" + std::to_string(i + 1) + ": " +
                                       it->second + " (" + idx_str + ")");
        }
    }
