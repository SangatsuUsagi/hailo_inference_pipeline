...
float ImagePostprocessorPalmDetection::calculate_scale(float min_scale, float max_scale,
...
std::vector<Anchor> ImagePostprocessorPalmDetection::generate_anchors(const nlohmann::json &opts) {
  ...
  while (layer_id < strides_size) {
    ...
    while (last_same < strides_size &&
      ...
      float scale = calculate_scale(opts["min_scale"].get<float>(), opts["max_scale"].get<float>(),
      ...
      if (last_same == 0 && opts["reduce_boxes_in_lowest_layer"].get<bool>()) {
        ...
        if (opts["interpolated_scale_aspect_ratio"].get<float>() > 0.0f) {
          float scale_next =
              (last_same == strides_size - 1)
                  ? 1.0f
                  : calculate_scale(opts["min_scale"].get<float>(), opts["max_scale"].get<float>(),
                                    last_same + 1, strides_size);
          scales.push_back(std::sqrt(scale * scale_next));
          aspect_ratios.push_back(opts["interpolated_scale_aspect_ratio"].get<float>());
        }
      }
      ++last_same;
    }
...
ImagePostprocessorPalmDetection::ImagePostprocessorPalmDetection(
    ...
    : scale_(params.first.first), pad_(params.second) {
...
ImagePostprocessorPalmDetection::decode_box(const float *raw, size_t anchor_idx) const {
...
ImagePostprocessorPalmDetection::tensors_to_detections(const std::vector<float> &raw_boxes,
...
float ImagePostprocessorPalmDetection::iou(const Detection &a, const Detection &b) const {
...
std::vector<ImagePostprocessorPalmDetection::Detection>
ImagePostprocessorPalmDetection::weighted_nms(std::vector<Detection> dets) const {
  if (dets.empty())
    return {};
  ...
  for (size_t i = 0; i < dets.size(); ++i) {
    ...
    if (overlapping.size() > 1) {
      ...
      for (float &kp : weighted.keypoints)
        kp /= total_score;
...
void ImagePostprocessorPalmDetection::denormalize_detections(std::vector<Detection> &dets) const {
  float xs = model_configs_["x_scale"].get<float>();
...
std::vector<ImagePostprocessorPalmDetection::Detection>
ImagePostprocessorPalmDetection::postprocess_palm_detection(const OutputMap &outputs) const {
...
cv::Mat
ImagePostprocessorPalmDetection::draw_detections(cv::Mat image,
...
cv::Mat ImagePostprocessorPalmDetection::draw_roi(cv::Mat image,
                                                  const std::vector<Detection> &dets) const {
  ...
  for (const auto &d : dets) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      pts[i] = cv::Point(static_cast<int>(cos_t * px - sin_t * py + xc),
...
cv::Mat ImagePostprocessorPalmDetection::postprocess(const cv::Mat &frame,
