...
preprocess_image_with_pad(const cv::Mat &image, int target_height,
                          int target_width) {
  cv::Mat rgb;
...
InferPipeline::InferPipeline(const std::string &net_path, int batch_size,
                             bool is_async, bool is_callback, bool is_nms,
  ...
  if (is_async_) {
    ...
    for (const auto &name : infer_model_->get_output_names()) {
      ...
      else if (std::ranges::find(layer_name_u16_, name) !=
               layer_name_u16_.end())
    ...
    auto cfg_params_exp =
        hef_->create_configure_params(HAILO_STREAM_INTERFACE_PCIE);
    if (!cfg_params_exp)
    ...
    auto in_p = network_group_->make_input_vstream_params(
        {}, HAILO_FORMAT_TYPE_UINT8, HAILO_DEFAULT_VSTREAM_TIMEOUT_MS,
    ...
    auto out_p = network_group_->make_output_vstream_params(
        {}, HAILO_FORMAT_TYPE_FLOAT32, HAILO_DEFAULT_VSTREAM_TIMEOUT_MS,
        HAILO_DEFAULT_VSTREAM_QUEUE_SIZE);
    if (!out_p)
...
void InferPipeline::infer_async(const std::vector<cv::Mat> &inputs) {
  ...
  for (const auto &out_name : infer_model_->get_output_names()) {
    ...
    if (!cfg_out_exp)
      throw InferenceSubmitError("Failed to get configured output stream '" +
                                 out_name + "'");
    size_t frame_size = cfg_out_exp->get_frame_size();

    bool is_u8 =
        std::ranges::find(layer_name_u8_, out_name) != layer_name_u8_.end();
    bool is_u16 =
        std::ranges::find(layer_name_u16_, out_name) != layer_name_u16_.end();
    ...
    if (is_nms_ || is_u8) {
      ...
      out_stream_exp->set_buffer(hailort::MemoryView(
          output_buffers_u16_[out_name].data(), frame_size));
    } else {
      output_buffers_f32_[out_name].assign(frame_size / sizeof(float), 0.0f);
      out_stream_exp->set_buffer(hailort::MemoryView(
          output_buffers_f32_[out_name].data(), frame_size));
    }
  }
  ...
  if (is_hailo_error(ready_status)) {
    ...
    throw InferenceSubmitError(
        "Hailo device error while waiting for async ready");
  }
...
OutputMap InferPipeline::wait_and_get_output() {
  if (!has_job_)
    throw std::runtime_error(
        "No inference job pending. Call inference() first.");
  ...
  if (is_hailo_error(status)) {
    if (is_hailo_timeout(status))
      throw InferenceTimeoutError("Inference job timed out after " +
                                  std::to_string(TIMEOUT_MS) + "ms");
    throw InferenceWaitError(
        "Hailo device error waiting for inference results");
  }
...
void InferPipeline::parse_nms_buffer(std::span<const uint8_t> buf,
...
void InferPipeline::parse_nms_buffer_raw(const std::vector<uint8_t> &raw,
                                         const std::string &name,
                                         InferenceOutput &out) {
