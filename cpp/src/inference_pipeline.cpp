...
bool is_hailo_timeout(hailo_status status) { return status == HAILO_TIMEOUT; }

bool is_hailo_error(hailo_status status) { return status != HAILO_SUCCESS; }
...
preprocess_image_with_pad(const cv::Mat &image, int target_height, int target_width) {
  ...
  cv::Mat padded;
  cv::copyMakeBorder(resized, padded, padh, padh2, padw, padw2, cv::BORDER_CONSTANT,
                     cv::Scalar(0, 0, 0));

  return {
      padded, {scale, scale}, {static_cast<int>(padh * scale), static_cast<int>(padw * scale)}};
}
...
InferPipeline::InferPipeline(const std::string &net_path, int batch_size, bool is_async,
                             bool is_callback, bool is_nms,
                             const std::vector<std::string> &layer_name_u8,
                             const std::vector<std::string> &layer_name_u16)
    : is_async_(is_async), is_callback_(is_callback), is_nms_(is_nms),
      layer_name_u8_(layer_name_u8), layer_name_u16_(layer_name_u16) {

  hailo_vdevice_params_t params = {};

  auto status = hailo_init_vdevice_params(&params);
  if (HAILO_SUCCESS != status) {
    throw std::runtime_error("Failed init vdevice_params, status = " + std::to_string(status));
  }

  params.scheduling_algorithm = HAILO_SCHEDULING_ALGORITHM_ROUND_ROBIN;
  auto vdev_exp = hailort::VDevice::create();
  if (!vdev_exp)
    throw std::runtime_error("Failed to create VDevice: " + std::to_string(vdev_exp.status()));
  ...
  if (is_async_) {
    ...
    auto in_p = network_group_->make_input_vstream_params({}, HAILO_FORMAT_TYPE_UINT8,
    ...
    auto out_p = network_group_->make_output_vstream_params({}, HAILO_FORMAT_TYPE_FLOAT32,
                                                            HAILO_DEFAULT_VSTREAM_TIMEOUT_MS,
                                                            HAILO_DEFAULT_VSTREAM_QUEUE_SIZE);
    if (!out_p)
...
void InferPipeline::infer_async(const std::vector<cv::Mat> &inputs) {
  ...
  for (size_t i = 0; i < input_names.size(); ++i) {
    ...
    auto in_stream_exp = bindings_.input(input_names[i]);
    if (!in_stream_exp)
      throw InferenceSubmitError("Failed to get input stream '" + input_names[i] + "'");
    auto status =
        in_stream_exp->set_buffer(hailort::MemoryView(cont.data, cont.total() * cont.elemSize()));
  ...
  // these member vectors, which we then return in
  // collect_output_from_bindings().
  ...
  for (const auto &out_name : infer_model_->get_output_names()) {
    ...
    auto cfg_out_exp = infer_model_->output(out_name);
    if (!cfg_out_exp)
      throw InferenceSubmitError("Failed to get configured output stream '" + out_name + "'");
    size_t frame_size = cfg_out_exp->get_frame_size();
    ...
    auto out_stream_exp = bindings_.output(out_name);
    if (!out_stream_exp)
      throw InferenceSubmitError("Failed to get output stream '" + out_name + "'");
    if (is_nms_ || is_u8) {
      // Raw byte buffer for NMS or explicit uint8 layers
      output_buffers_u8_[out_name].assign(frame_size, 0);
      out_stream_exp->set_buffer(
  ...
  auto job_exp = [&]() -> hailort::Expected<hailort::AsyncInferJob> {
    if (is_callback_) {
  ...
  if (!job_exp)
    throw InferenceSubmitError("Failed to submit async job: " + std::to_string(job_exp.status()));
...
OutputMap InferPipeline::wait_and_get_output() {
  ...
  if (is_hailo_error(status)) {
    if (is_hailo_timeout(status))
      throw InferenceTimeoutError("Inference job timed out after " + std::to_string(TIMEOUT_MS) +
                                  "ms");
...
// Assemble OutputMap from the member buffer vectors (called after job
// completion).
...
OutputMap InferPipeline::infer_pipeline(const std::vector<cv::Mat> &inputs) {
  ...
  auto in_streams_exp =
      hailort::VStreamsBuilder::create_input_vstreams(*network_group_, input_vstream_params_);
  if (!in_streams_exp)
    throw InferencePipelineError("Failed to create input vstreams: " +
                                 std::to_string(in_streams_exp.status()));

  auto out_streams_exp =
      hailort::VStreamsBuilder::create_output_vstreams(*network_group_, output_vstream_params_);
  if (!out_streams_exp)
  ...
  for (size_t i = 0; i < in_streams.size(); ++i) {
    cv::Mat cont = inputs[i].isContinuous() ? inputs[i] : inputs[i].clone();
    auto wst = in_streams[i].write(hailort::MemoryView(cont.data, cont.total() * cont.elemSize()));
    if (is_hailo_error(wst))
      throw InferencePipelineError("Failed to write input vstream " + std::to_string(i) + ": " +
                                   std::to_string(wst));
  }
  ...
  for (auto &out_stream : out_streams) {
    ...
    if (is_nms_) {
      ...
      if (is_hailo_error(rst))
        throw InferencePipelineError("Failed to read NMS output: " + std::to_string(rst));
      parse_nms_buffer_raw(raw, name, out);
    } else {
      size_t float_count = frame_bytes / sizeof(float);
      out.regular.data.resize(float_count, 0.0f);
      auto rst = out_stream.read(hailort::MemoryView(out.regular.data.data(), frame_bytes));
      if (is_hailo_error(rst))
        throw InferencePipelineError("Failed to read output vstream '" + name +
                                     "': " + std::to_string(rst));
    }
...
void InferPipeline::parse_nms_buffer(std::span<const uint8_t> buf, const std::string & /*name*/,
                                     InferenceOutput &out) {
  size_t offset = 0;
...
void InferPipeline::parse_nms_buffer_raw(const std::vector<uint8_t> &raw, const std::string &name,
                                         InferenceOutput &out) {
